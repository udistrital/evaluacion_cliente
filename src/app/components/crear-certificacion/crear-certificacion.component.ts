...
export class CrearCertificacionComponent implements OnInit {
  ...
  crearPdf() {
    ...
    this.nuxeoService.getDocumentos$(arreglo, this.documentoService).pipe(take(1)).subscribe(
      (response) => {
        //console.log("esta es la respuesta de nuxeo", response["Enlace"]);
        
        pdf.add(
          new Table([
            [docDefinition.escudoImagen, docDefinition.valorCabe, new Txt("código de autenticidad:" + response["Enlace"])
            .bold()
            .alignment("right")
            .fontSize(9).end,]
